...
from util import (
    ...
    get_zzz_acc_by_name,
    hoyolab_client_init,
    get_skport_common_headers,
    get_skport_cred,
    generate_skport_sign,
)
import requests
...
KIRANYAN = (
    "<:kiranyan1:1126353426880667688><:kiranyan2:1126353457570402364>" * 3
    + "<:kiranyan3:1126353472065917028>"
)
...
class Notes(commands.Cog):
    ...
    async def notes(self, ctx: Context, name: str = None):
        ...
        try:
            genshin_account = (
                get_genshin_acc_by_name(name)
                if name is not None
                else get_genshin_acc_by_discord_id(ctx.author.id)
            )
            ...
            embed.set_image(
                url="https://media.discordapp.net/attachments/375466192397402124/927855237014884352/unknown.png"
            )
        ...
        try:
            starrail_account = (
                get_starrail_acc_by_name(name)
                if name is not None
                else get_starrail_acc_by_discord_id(ctx.author.id)
            )
        ...
        try:
            zzz_account = (
                get_zzz_acc_by_name(name)
                if name is not None
                else get_zzz_acc_by_discord_id(ctx.author.id)
            )
    ...
    async def genshin_notes(self, ctx: Context, genshin_account: HoyolabAccount):
        try:
            ...
            if int(notes.remaining_resin_recovery_time.total_seconds()) == 0:
                ...
                maxout_time = (
                    datetime.datetime.now() + notes.remaining_resin_recovery_time
                )
            ...
            for exp in notes.expeditions:
                if exp.status == "Ongoing":
                    expdone_time = (
                        datetime.datetime.now() + exp.remaining_time
                    ).strftime("%I:%M %p")
                    expedition_counter[expdone_time] = (
                        expedition_counter[expdone_time] + 1
                        if expdone_time in expedition_counter
                        else 1
                    )
                elif exp.status == "Finished":
                    expedition_counter["now"] = (
                        expedition_counter["now"] + 1
    ...
    async def starrail_notes(self, ctx: Context, starrail_account: HoyolabAccount):
        try:
            ...
            if notes.current_train_score != notes.max_train_score:
                desc += " **Daily training not completed!** <:nonoseganyu:927411234226176040>"
            ...
            for exp in notes.expeditions:
                if exp.status == "Ongoing":
                    ...
                    expdone_time = (
                        datetime.datetime.now() + datetime.timedelta(seconds=ts)
                    ).strftime("%I:%M %p")
                    assignment_counter[expdone_time] = (
                        assignment_counter[expdone_time] + 1
                        if expdone_time in assignment_counter
                        else 1
                    )
                elif exp.status == "Finished":
                    assignment_counter["now"] = (
                        assignment_counter["now"] + 1
    ...
    async def zzz_notes(self, ctx: Context, zzz_account: HoyolabAccount):
        try:
            ...
            if notes.battery_charge.seconds_till_full == 0:
                ...
                maxout_time = datetime.datetime.now() + datetime.timedelta(
                    seconds=notes.battery_charge.seconds_till_full
                )
                desc += maxout_time.strftime(" (Maxout - %I:%M %p)")
            ...
            if notes.engagement.current != notes.engagement.max:
                desc += " **Daily engagement not completed!** <:nonoseganyu:927411234226176040>"
    ...
    def get_endfield_notes(self, account: SKPortAccount):
        cred = get_skport_cred(account)
        headers = get_skport_common_headers()
        headers["cred"] = cred

        # Generate signV2
        sign = generate_skport_sign(account, "/card/detail")

        url = f"https://zonai.skport.com/api/v1/game/endfield/card/detail?serverId=3&userId=7935425120387"
        retry = False
        try:
            headers = {
                "cred": "rMORZ6Pl6L2syxIysHPvrU8rpZ0molR4",
                "sk-language": "en",
            }
        ...
        if retry:
            # force get_skport_cred and retry same request
            cred = get_skport_cred(account, True)
            headers = {
                "cred": cred,
                "sk-language": "en",
            }
